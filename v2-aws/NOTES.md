...
**Verified, not assumed:** assuming the role and attempting `s3:PutObject` returns
`AccessDenied` — "because no identity-based policy allows the s3:PutObject action".
No explicit `Deny` exists anywhere in the policy; the write fails because the
permission was never granted. Withholding a permission means omitting it, not
denying it. `ListBucket` under the same session succeeds.

---

## Verification (26 Sep 2026)

Applying is not verifying. A green apply proves AWS accepted a configuration;
it proves nothing about behaviour. Each control was exercised:

- **Log delivery** — objects present under `AWSLogs/<account>/CloudTrail/ap-southeast-1/`.
- **Multi-region** — digest files appeared under **16 region prefixes** for a trail
  created in one region. `is_multi_region_trail` demonstrated by output, not asserted
  from the config file.
- **Log file validation** — CloudTrail only produces digest files when validation is
  enabled, so their presence is the evidence.
- **KMS in the path** — `head-object` reports `ServerSideEncryption: aws:kms` with
  `SSEKMSKeyId` matching the `alias/gcloud-v2` key. Had it reported `AES256`, the
  whole KMS section would have been theory.
- **Least privilege** — see the IAM section above. A policy that has never refused
...
**Applying is not verifying.** Every control in this file was confirmed by observing
behaviour, not by reading back the configuration that produced it.
